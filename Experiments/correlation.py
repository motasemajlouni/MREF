...
import numpy as np
import pandas as pd
from scipy.stats import zscore, pearsonr,spearmanr, t
...
def compute_direct_assessment_correlations_pearson(df_human_scores, df_metrics_scores, aspect, segment_id_cols):
    df_da_scores = df_human_scores.reset_index()
    cols_of_interest = segment_id_cols + [aspect]
    ...
    correlations_data = []
    for metric in metrics_names:
        corr, p_value = pearsonr(df_all_scores[metric], df_all_scores[aspect])
    ...
    return df_correlations_metrics_human, None

def compute_direct_assessment_correlations_spearman(df_human_scores, df_metrics_scores, aspect, segment_id_cols):
    ...
    correlations_data = []
    for metric in metrics_names:
        corr, p_value = spearmanr(df_all_scores[metric], df_all_scores[aspect])
